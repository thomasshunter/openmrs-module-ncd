package org.openmrs.module.ncd.output.datasource;
...
/**
 * The scheduled task that implements the Data Source Report
 * @author ehorstkotte
 *
 */
public class DataSourceReportTask implements Task {

    private static Log log = LogFactory.getLog(DataSourceReportTask.class);
    ...
    public final static String PROP_NAME_PERIOD = "DataSourceReportTask.period";
    /** The default sampling period, in days */
    public final static int DEFAULT_PERIOD = 7;
    
    /** The task definition property under which the cutoff date/time for the
     * report/export is stored. */
    public final static String PROP_NAME_CUTOFF_DATETIME = "DataSourceReportTask.cutoffDateTime";

    // Constants for fiddlage
    private static final String DEFAULT_FAILURE_SUBJECT = "[NCD-ERROR] Data source export failed";
    private static final String DEFAULT_SUCCESS_SUBJECT = "[NCD-SUCCESS] Data source export succeeded";
    ...
    public void execute() {
        ...
        try {
            ...
            samplingWindow.setLow(DateUtilities.adjust(samplingWindow.getHigh(), Calendar.DAY_OF_YEAR, -samplingPeriod));
            ...
            DataSourceReportFilter filter = new DataSourceReportFilter();
            filter.setSamplingWindow(samplingWindow);
            ...
            log.debug("execute: gather data source data");
            
            // Get the report data matching the filter.
            List<DataSourceInfo> dataSources =
            	cds.findDataSourceInfo(config, filter);
            ...
            // We're exporting
            	...
            	config.setProperty(DataFeedSinkFactory.PROP_SINK_OUTPUT_COLS_TO_INCL, DataSourceInfo.getColumnNames());
    	        ...
    	        for (DataSourceInfo row : dataSources) {
    ...
    private String format(Map<String, Object> data) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        DataFormatter formatter = DataFormatterFactory.getInstance(config.getProperties(), "datasource.template");
    ...
    private void recordStatus(long startTime, long stopTime, boolean error) {
    	...
    	String message;
    	if (!error) {
    		message = "Data source report succeeded: generated report in " + ((stopTime - startTime)/1000) + " seconds";
    	} else {
    		message = "Data source report failed (check ncd.log for exceptions)";
    	}
