...
/** Form controller for the institutionDetail.jsp page */
@SuppressWarnings("deprecation")
public class InstitutionDetailFormController extends SimpleFormController {
    ...
    protected boolean isDuplicate(Institution form) {

    	Institution institution = 
    		NCDUtilities.getService().findInstitutionByName(form.getName());
    	log.debug("institution=" + institution);
    	log.debug("form=" + form);
        return institution != null && !institution.getId().equals(form.getId());
    }
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object object, BindException exceptions) throws Exception {
		...
		Institution theForm = (Institution) object;
        ...
        if (action.equals(msa.getMessage("ncd.buttons.cancel"))) {
            ...
            // If a distinct duplicate exists
                ...
                httpSession.setAttribute(WebConstants.OPENMRS_ERROR_ATTR, "ncd.pages.institutionDetail.error.duplicate");
                ...
                // persist the form
                NCDUtilities.getService().saveInstitution(theForm);
                ...
                if (theForm.getId() == null) {
                    httpSession.setAttribute(WebConstants.OPENMRS_MSG_ATTR, "ncd.pages.institutionDetail.error.added");
                }
                else {
                    httpSession.setAttribute(WebConstants.OPENMRS_MSG_ATTR, "ncd.pages.institutionDetail.error.updated");
                }
     ...
     */
    @Override
	protected Institution formBackingObject(HttpServletRequest request) throws Exception {

        // If the request has an "edit" parameter
        String idStr = request.getParameter("edit");
        if (idStr != null) {

            return new Institution(NCDUtilities.getService().getInstitution(Long.parseLong(idStr)));
        }
        else {
